...
**Q2 - Bind the full extension map, or per-extension commitments?****Per-extension commitments**, aggregated. Compute `sha256(JCS(ext_i))` for each
...
**Q4 - Explicit actor type on `actor_id`?****Yes - typed prefix, same pattern:**`agent:`, `svc:`, `human:`. The replay/policy
