...
fn main() {
        ...
        let r = catch_unwind(AssertUnwindSafe(|| {
            ...
            match rng.upto(8) {
                ...
                0 => {
                    ...
                    let _ = has_trailer(&b);
                    let _ = trailer_sig(&b);
                    let _ = verify_pq(&b, &kp.public);
                    let _ = verify_hybrid(&b, &vk, &kp.public);
                }
                // valid core, corrupted trailer header
                1 => {
                    let mut b = good_hybrid.clone();
                    for _ in 0..rng.upto(8) {
                        let p = 128 + rng.upto(16);
                        b[p] = rng.byte();
                    }
                    let _ = trailer_sig(&b);
                    let _ = verify_pq(&b, &kp.public);
                ...
                3 => {
                    let n = rng.upto(good_hybrid.len() + 1);
                    let b = &good_hybrid[..n];
                    let _ = trailer_sig(b);
                    let _ = verify_pq(b, &kp.public);
                    let _ = verify_hybrid(b, &vk, &kp.public);
                ...
                5 => {
                    ...
                    if !l.is_empty() {
                        let i = rng.upto(l.len());
                        if let Some(p) = inclusion_proof(&l, i) {
                            let _ = verify_inclusion(&l[i], &p, &root);
                        }
                    }
                }
                ...
                6 => {
                    let k = rng.upto(200);
                    let s = String::from_utf8_lossy(&rng.bytes(k)).to_string();
                    let _ = merkle_leaf(&s);
                    let _ = merkle_node(&s, &s);
                }
                // oversized declared length with real bytes after
                _ => {
                    let mut b = good_core.to_vec();
                    b.extend_from_slice(b"AETHPQ01");
                    b.push(rng.byte());
                    b.extend_from_slice(&[0u8; 3]);
                    b.extend_from_slice(&(rng.next() as u32).to_le_bytes());
                    let k = rng.upto(4000);
                    b.extend(rng.bytes(k));
                    let _ = trailer_sig(&b);
                    let _ = verify_pq(&b, &kp.public);
                    let _ = verify_hybrid(&b, &vk, &kp.public);
                }
    ...
    for i in 0..50_000usize {
        ...
        let n = rng.upto(4000);
        let g = rng.bytes(n);
        if verify_pq(&g, &kp.public) {
            accepted_garbage += 1;
        }
        if verify_hybrid(&g, &vk, &kp.public) {
            accepted_garbage += 1;
        }
    }
    ...
    if panics > 0 || accepted_garbage > 0 {
        std::process::exit(1);
    }
