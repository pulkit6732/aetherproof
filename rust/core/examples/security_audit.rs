...
fn main() {
    ...
    let (before, after) = {
        ...
        let probe = |addr: *const u8, n: usize| -> usize {
            ...
            unsafe {
                ...
                for w in sl.windows(32) {
                    if w.iter().all(|&b| b == MARK) {
                        runs += 1;
                    }
                }
            }
    ...
    let base = [0x5Au8; 64];
    let mut early = base;
    early[0] ^= 0xFF; // differs at byte 0
    let mut late = base;
    late[63] ^= 0xFF; // differs at byte 63
    const R: usize = 3_000_000;

    // warm up
    for _ in 0..100_000 {
        std::hint::black_box(ct_eq(&base, &early));
    }
    ...
    let t = Instant::now();
    for _ in 0..R {
        std::hint::black_box(ct_eq(&base, &late));
    }
    ...
    println!(
        "   {}",
        if ct_ok {
            "PASS  no position-dependent timing above noise"
        } else {
            "REVIEW skew above threshold"
        }
    );
    ...
    println!(
        "   {}",
        if rejected {
            "PASS  fails closed at build time, not verify time"
        } else {
            "FAIL"
        }
    );
