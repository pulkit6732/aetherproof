...
fn main() {
    ...
    for _ in 0..50_000 { std::hint::black_box(ct_eq(&base, &probe)); }

    let mut time_it = |probe: &[u8; 64]| {
        let t = Instant::now();
        for _ in 0..INNER { std::hint::black_box(ct_eq(&base, probe)); }
    ...
    for r in 0..ROUNDS {
        ...
        let mut sample = |which: usize| -> f64 {
            probe = base;
            match which { 0 => probe[0] ^= 0xFF, 1 => probe[63] ^= 0xFF, _ => {} }
            time_it(&probe)
        };
        let (a, b, c) = (order, (order + 1) % 3, (order + 2) % 3);
        let va = sample(a); let vb = sample(b); let vc = sample(c);
        for (which, v) in [(a, va), (b, vb), (c, vc)] {
            match which { 0 => t0.push(v), 1 => t63.push(v), _ => teq.push(v) }
        }
    }

    let m0 = median(&mut t0); let m63 = median(&mut t63); let meq = median(&mut teq);
    ...
    } else { "INVESTIGATE  skew exceeds noise" });
    println!("\n  Note: a short-circuit leak makes an early difference FASTER.");
    println!("  Here byte0 vs byte63 is {}.",
        if m0 < m63 * 0.98 { "faster — consistent with a leak" }
        else if m0 > m63 * 1.02 { "slower — inconsistent with a leak" }
