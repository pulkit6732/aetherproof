...
use aetherproof_core::{
    generate, to_bytes, from_bytes, verify,
    tamper_probe, dev_signing_key, dev_verifying_key,
    fnv1a, RECEIPT_SIZE,
};
...
fn cmd_generate(args: &[String]) {
    ...
    while i < args.len() {
        match args[i].as_str() {
            "--binary"        => { i += 1; binary_path   = Some(args[i].clone()); }
            "--pid"           => { i += 1; pid           = args[i].parse().unwrap_or(1); }
            "--memory-hash"   => { i += 1; memory_hash   = parse_u64_hex_or_dec(&args[i]); }
            "--syscall-count" => { i += 1; syscall_count = args[i].parse().unwrap_or(0); }
    ...
    println!("binary_hash   = 0x{:016X}  (FNV-1a of {} bytes)", fnv1a(&binary), binary.len());
    process::exit(if valid { 0 } else { 1 });
}
...
fn cmd_self_test() {
    ...
    macro_rules! check {
        ($name:expr, $cond:expr) => {
            ...
            else      { eprintln!("  FAIL  {}", $name); fail += 1; }
        };
    }
    ...
    check!("T3: round-trip binary_hash",   r1.binary_hash   == r2.binary_hash);
    check!("T3: round-trip syscall_count", r1.syscall_count == r2.syscall_count);
    ...
    for i in 0..64usize {
        let mut t = b1;
        t[i] ^= 0x55;
        if verify(&t, &vk) { all_meta_flip_invalid = false; }
    }
    check!("T5: flip any metadata byte → INVALID (64/64)", all_meta_flip_invalid);
    ...
    for i in 64..128usize {
        let mut t = b1;
        t[i] ^= 0xAA;
        if verify(&t, &vk) { all_sig_flip_invalid = false; }
    }
    check!("T6: flip any sig byte → INVALID (64/64)", all_sig_flip_invalid);
    ...
    check!("T8: fnv1a('foobar') == known vector", fnv1a(b"foobar") == 0x85944171f73967e8);

    // T9: different binaries → different hashes
    let ra = generate(1, b"model_a", 0, 0, &sk);
    let rb = generate(1, b"model_b", 0, 0, &sk);
    check!("T9: distinct binaries → distinct hashes", ra.binary_hash != rb.binary_hash);

    // T10: memory_hash field is preserved
    let rh = generate(1, b"m", 0xABCD_1234_5678_CAFE, 0, &sk);
    check!("T10: memory_hash preserved", rh.memory_hash == 0xABCD_1234_5678_CAFE);
...
fn cmd_bench(args: &[String]) {
    ...
    println!("verified: {verified}/{count} ({:.1}%)", verified as f64 / count as f64 * 100.0);

    // Tamper all
    let t2 = Instant::now();
    let mut tamper_ok = 0u64;
    for b in &receipts {
        if tamper_probe(b, &vk) { tamper_ok += 1; }
    }
...
fn print_receipt_table(r: &aetherproof_core::Receipt, valid: bool) {
    ...
    println!("│  binary_hash   : 0x{:016X}               │", r.binary_hash);
    println!("│  memory_hash   : 0x{:016X}               │", r.memory_hash);
    println!("│  syscall_count : {:<38}│", r.syscall_count);
    println!("│  entropy_seed  : 0x{:08X}                           │", r.entropy_seed);
    println!("│  timestamp_ns  : {:<38}│", r.timestamp_ns);
    println!("│  binary_len    : {:<38}│", r.binary_len);
    print!("│  sig[0..8]     : ");
    for b in &r.sig[0..8] { print!("{b:02X}"); }
    println!("...                          │");
    println!("├─────────────────────────────────────────────────────────┤");
    if valid {
